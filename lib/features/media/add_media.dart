...
class _AddMediaScreenState extends State<AddMediaScreen> {
    ...
    if (pickedFiles != null) {
      for (var file in pickedFiles) {
        if (file.path.toLowerCase().endsWith(".png") ||
          ...
          if (!alreadyExists && _selectedImages.length < 5) {
            setState(() {
  ...
  Future<void> _pickVideo() async {
    ...
    if (pickedFile != null) {
      ...
      if (!alreadyExists && _selectedVideos.length < 5) {
        setState(() {
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                ...
                SizedBox(height: 8),
                Text(
                  'Max 5 files can be selected at a time (Max 50MB each)',
                  style: TextStyle(
                    color: Colors.red,
                    fontSize: 12,
                  ),
                            ...
                            ElevatedButton(
                              onPressed: () {
                                if (_selectedImages.length >= 5) {
                                  _showAutoDismissDialog("Error", "Maximum 5 files allowed");
                                  return;
                                }
                                _pickImage();
                              },
